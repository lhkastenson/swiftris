...
//  my_first_ios_game
//
//  Created by Lon Kastenson on 4/3/17.
//  Copyright © 2017 Lon Kastenson. All rights reserved.
//
...
class GameScene: SKScene {
    ...
    func redrawShape(shape:Shape, completion:@escaping () -> ()) {
        for block in shape.blocks {
            let sprite = block.sprite!
            let moveTo = pointForColumn(column: block.column, row: block.row)
            let moveAction: SKAction = SKAction.move(to: moveTo, duration: 0.05)
            moveAction.timingMode = .easeOut
    ...
    func animateCollapsingLines(linesToRemove: Array<Array<Block>>, fallenBlocks: Array<Array<Block>>, completion: @escaping () -> ()) {
        ...
        for rowToRemove in linesToRemove {
            for block in rowToRemove {
                ...
                sprite.run(SKAction.sequence([archAction, SKAction.fadeOut(withDuration: TimeInterval(randomDuration)), SKAction.removeFromParent()]))
            }
        }
